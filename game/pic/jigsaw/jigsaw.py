...
class JigsawPuzzleApp(TkinterFrame):
    def __init__(self):
        ...
        if self.config_manager.get_value("image_file") is None:
            self.config_manager.set_value("image_file", "image/puzzle.png")
            need_save = True
        
        if self.config_manager.get_value("effect_file") is None:
            self.config_manager.set_value("effect_file", "sound/effect.wav")
            need_save = True
        
        if self.config_manager.get_value("bg_file") is None:
            self.config_manager.set_value("bg_file", "sound/background2.mp3")
            need_save = True
        
        if need_save:
            self.config_manager.save()
        
        self.image_file = os.path.join(curr_path, self.config_manager.get_value("image_file"))
        ...
        pygame.mixer.music.load(self.bg_sound_file)
    ...
    def update_title(self):
        ...
        self.title(f"직소 퍼즐 [{file_name}] - 난이도: {self.cols}x{self.rows}")
    ...
    def create_menu(self):
        ...
        menubar.add_cascade(label="동영상", menu=movie_menu)

        # 클릭 시 해당 볼륨으로 변경
        movie_menu.add_command(label="동영상 보기", command=lambda: self.open_video_window())
        bgs_menu.add_separator()
    ...
    def open_video_window(self):
        ...
        # 완전히 독립된 동영상 플레이어 서브 윈도우 생성 (self는 부모인 퍼즐창)
        self.video_win = VideoPlayerWindow(self)
