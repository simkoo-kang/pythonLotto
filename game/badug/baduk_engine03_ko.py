...
class BadukEngine:
    def __init__(self, size=13):
        ...
        # [⭐ 패 규칙 추가] 현재 턴에 착수할 수 없는 패(Ko) 좌표를 기억 (없으면 None)
    ...
    def remove_dead_stones(self, target_color, last_move_x, last_move_y):
        """
        [⭐ 패 규칙 연동을 위해 수정됨]
        돌을 따내면서, 방금 발생한 상황이 '패' 조건에 부합하는지 정밀 판별합니다.
        """
        visited = set()
        captured_count = 0
        captured_stones = []
        ...
        # [핵심 패 판별 알고리즘]
        # 방금 놓은 돌이 단 1개짜리 그룹이고, 따낸 상대 돌도 정확히 '단 1개'라면 '패(Ko)' 상황입니다.
        my_color = 2 if target_color == 1 else 1
        my_group, _ = self.find_group_and_liberties(last_move_x, last_move_y)

        if captured_count == 1 and len(my_group) == 1:
            # 상대가 다음 턴에 즉시 되따낼 수 없는 '패 금지 좌표'로 지정합니다.
            self.ko_coordinate = captured_stones[0]
        else:
            # 패 상황이 아니거나 국면이 풀렸으므로 패 제한을 해제합니다.
            self.ko_coordinate = None
...
class BadukTsumegoGUI:
    def __init__(self, root, board_size=13):
        ...
        try:
            ...
            if os.path.exists(os.path.join(current_dir, "../sound/stone.mp3")):
                self.stone_sound = pygame.mixer.Sound(
                    os.path.join(current_dir, "../sound/stone.mp3")
                )
    ...
    def update_status_by_mode(self):
        ...
        print(f"update_status_by_mode {mode}")
        if mode == "normal":
    ...
    def on_board_click(self, event):
        """바둑판 마우스 왼쪽 클릭 시 (자살수 + 패 규칙 전면 적용)"""
        ...
        # 지우개 모드이거나, 돌이 있는 곳을 다시 누르면 제거 (이때 패 좌표도 리셋)
        ...
        # [일반 대국 모드]
            ...
            # 1. [⭐ 규칙 검사 1] 현재 클릭한 자리가 패(Ko)로 묶인 금지 자리인지 확인
            if self.engine.ko_coordinate == (x, y):
                messagebox.showwarning(
                    "패(劫) 착수 금지",
                    "패 규칙에 의해 지금 즉시 되따낼 수 없습니다!\n다른 곳에 한 수 둔 뒤(패감 사용) 따내야 합니다.",
                )
                return
            ...
            if self.engine.is_suicide_move(x, y, self.current_turn):
                messagebox.showwarning(
                    "착수 금지",
                    "바둑 규칙상 자살수는 둘 수 없습니다!\n(단, 상대 돌을 따낼 수 있는 경우는 예외)",
                )
                return
            ...
            # 따내기 함수 호출 시 방금 둔 좌표(x, y)를 넘겨주어 패를 판별하게 합니다.
...
if __name__ == "__main__":
    window = tk.Tk()
    BadukTsumegoGUI(window, board_size=19)
    window.mainloop()
