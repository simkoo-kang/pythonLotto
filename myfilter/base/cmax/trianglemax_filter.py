...
class TriangleMaxFilter(AMaxFilter):
    ...
    @property
    def name(self) -> str: return "삼각 패턴 Filter"

    @property
    def description(self) -> str: return f"삼각 패턴의 최대 값이 {self.max_val}보다 작은지 검사"

    def filter(self, numbers: List[int]) -> bool:
        match_count = lotto_util.get_match_count_with_target_list(numbers, self.triangle_group)
