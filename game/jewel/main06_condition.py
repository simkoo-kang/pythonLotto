...
import pygame
import io
from game.jewel.settings import BGM, POP_WAV, WIDTH, HEIGHT, SCORE_PANEL_HEIGHT, GRID_SIZE, CELL_SIZE, EMPTY_COLOR, COLORS, MAX_MOVES, TARGET_SCORE
...
pygame.display.set_caption("🚀 완벽체 객체지향 퍼즐 게임 🚀")
...
# 💡 [초기화 및 리셋 함수 분리] R키 누를 때 새 게임으로 완전 리셋하기 위함
def reset_game():
    global board, current_score, current_combo, left_moves, game_result, state
    global mouse_down_pos, active_gem, destroyed, items, particles
    board = Board()
    current_score = 0
    current_combo = 0
...
while True:
    ...
    screen.blit(game_font.render(f"점수: {Str.number_format(current_score)} / {Str.number_format(TARGET_SCORE)}", True, (255, 255, 255)), (15, 12))
    screen.blit(game_font.render(f"남은 횟수: {left_moves}회", True, (255, 235, 50) if left_moves > 5 else (255, 50, 50)), (15, 42))
    if current_combo > 0: screen.blit(game_font.render(f"{current_combo} COMBO!", True, (50, 255, 255)), (420, 25))
    
    # 💡 [완벽 동결 및 엔딩 UI 패널]
    if game_result is not None:
        if current_score >= TARGET_SCORE: game_result = "CLEAR"
        elif left_moves <= 0 and state == "READY": game_result = "GAMEOVER"
    ...
    if game_result is not None:
        ...
        if game_result == "CLEAR": msg = result_font.render("STAGE CLEAR! 🌟", True, (50, 255, 100))
        else: msg = result_font.render("GAME OVER 💀", True, (255, 50, 50))
    ...
    if state == "SWAPPING" and not animating:
        ...
        is_item_trigger = (board.grid[r1][c1].item_type != "NORMAL" or board.grid[r2][c2].item_type != "NORMAL")
        
        if is_item_trigger or destroyed:
            if (r1, c1) not in destroyed and board.grid[r1][c1].item_type != "NORMAL": destroyed.append((r1, c1))
            if (r2, c2) not in destroyed and board.grid[r2][c2].item_type != "NORMAL": destroyed.append((r2, c2))
        ...
        for r, c in list(exploded):
            if board.grid[r][c].item_type != "NORMAL": board.trigger_item_effect(r, c, board.grid[r][c].item_type, rainbow_color_target, exploded, rainbow_item_target)
        for r, c in exploded:
            p_color = board.grid[r][c].color
            if p_color != EMPTY_COLOR and p_color in COLORS:
                cx, cy = c * CELL_SIZE + CELL_SIZE // 2, r * CELL_SIZE + SCORE_PANEL_HEIGHT + CELL_SIZE // 2
                ...
                for _ in range(20): 
                    ...
                    p.vy = random.uniform(-11, -1)     # 위쪽으로 솟구치는 분수 효과 강화 (-6~2 ➡️ -11~-1)
                ...
                # for _ in range(20):
                #     p = Particle(cx, cy, p_color)
                #     p.vx, p.vy, p.gravity = random.uniform(-8, 8), random.uniform(-11, -1), 0.35
                #     particles.append(p)
        
        combo_bonus = current_combo * 5
        for r, c in exploded: board.grid[r][c].color, board.grid[r][c].item_type, current_score = EMPTY_COLOR, "NORMAL", current_score + (10 + combo_bonus)
        ...
        if destroyed:
            ...
            if match_sound: match_sound.play()
            state = "CLEARING"
        else: current_combo, state = 0, "READY"


    for event in pygame.event.get():
        if event.type == pygame.QUIT: pygame.quit(); sys.exit()
        
        # 💡 [버그 완벽 수정 - 이중 잠금] game_result가 None일 때(게임 진행 중일 때)만 마우스 클릭 허용!
        # 점수가 초과되어 "CLEAR" 상태가 되는 순간, 마우스 누르기 감지를 실시간으로 '원천 차단'합니다.
        elif event.type == pygame.MOUSEBUTTONDOWN and state == "READY" and game_result is None:
            mx, my = pygame.mouse.get_pos()
            if my > SCORE_PANEL_HEIGHT:
                c, r = mx // CELL_SIZE, (my - SCORE_PANEL_HEIGHT) // CELL_SIZE
                if 0 <= r < GRID_SIZE and 0 <= c < GRID_SIZE:
                    # 💡 [버그 완벽 수정 - 초고속 클릭 기폭] 
                    # 특수 아이템인 경우, 뗄 때까지 기다리지 않고 누르는 그 즉시 폭발 상태로 강제 전환시킵니다!
                    if board.grid[r][c].item_type != "NORMAL":
                        rainbow_color_target, rainbow_item_target = (255, 255, 255), "NORMAL"
                        destroyed = [(r, c)]
                        left_moves -= 1
                        state = "CLEARING"
                        mouse_down_pos, active_gem = None, None  # 드래그 예약 원천 취소
                    else:
                        # 일반 보석인 경우에만 사방 드래그(스와이프)를 위한 준비 상태로 진입
                        mouse_down_pos = (mx, my)
                        active_gem = (r, c)

        # 💡 [버그 완벽 수정 - 이중 잠금] 마우스 이동(드래그) 역시 게임 진행 중일 때만 작동하도록 안전장치 강화
        elif event.type == pygame.MOUSEMOTION and mouse_down_pos and state == "READY" and game_result is None:
            mx, my = pygame.mouse.get_pos()
